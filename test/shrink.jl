...
using Enzyme.Shrink: Shrink, CAPTURING, SESSION, PASS, classify, Value, Arg, Call, Program, Oracle,
    ...
    remove_defs, drop_activity, shrink_arrays, simplify_call, probes, check, WrongDerivative, Class, sites, rewrite,
    reduce_definitions
...
@testset "Describing a call" begin
    m = @__MODULE__
    dx = [5.0, 5.0]
    c = Shrink.describe(set_err_if_func_written(Reverse), Const(cf), Active, (Duplicated(cx, dx), Const(cp)), m, mktempdir())
    ...
    c = Shrink.describe(set_runtime_activity(Forward), Const(cf), Duplicated, (BatchDuplicated([5.0], ([0.0], [1.0])), Const(cx)), m, mktempdir())
    @test call_line(c, "") == "autodiff(set_runtime_activity(Forward), cf, Duplicated, BatchDuplicated(a1, (da1_1, da1_2,)), Const(cx))"
    @test Shrink.describe(Reverse, Duplicated(cf, cf), Active, (Const(cp),), m, mktempdir()) isa String
    k = 5
    c = Shrink.describe(Reverse, Const(v -> v[k] * 2), Active, (Duplicated(cx, dx),), m, mktempdir())
    @test Meta.isexpr(c.f, :->) && [a.name for a in c.args] == [:cx, :k] && c.args[2].val.text == "5"
    q = name_the_function(Program(Any[], c, IdDict{Any, Vector{Value}}()))
    @test fparams(q.defs[end]) == [:v, :k] && q.call.f == :entry
...
@testset "Definitions" begin
    d = :(m = build([1.0, 2.0, 3.0], 40, (1, 2)))
    found = sites(d)
    @test [e for (_, e) in found if e isa Integer] == [40, 2]
    @test rewrite(d, [[2, 3] => 1]) == :(m = build([1.0, 2.0, 3.0], 1, (1, 2)))
    p = Program(Any[:(using Enzyme), d], Call(Reverse, :f, Active, Arg[]), IdDict{Any, Vector{Value}}())
    labels = first.(reduce_definitions(p))
    @test any(startswith("in `m = build("), labels) && any(endswith("2 integers → 1"), labels) && any(contains("drop 2 of 3 in vect"), labels)
end
...
@testset "Isolation" begin
    ...
    o = Oracle(dir; timeout = 10, workers = 2)
    try
        @test [c.kind for (_, c) in o([program(:(ccall(:abort, Cvoid, ()))), program(:(sleep(60)))])] == [:crash, :timeout]
