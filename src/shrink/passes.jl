# Passes: each proposes smaller programs as `label => program`, coarse cuts first.
...
Consecutive runs of `items` at every power-of-two size, largest first:
`chunks([2, 3, 5]) == [[2, 3], [5], [2], [3]]`. With `singles`, only the runs of one.
"""
...
# Keep the first `k` statements of the entry function and return the `k`-th's value, `k` in bisection order.
...
"""
    placeholder_statements(p; singles = false)

Remove chunks of statements from every named function, the entry first. An assignment of
the entry with a recorded value becomes `y = y_rec`, with `y_rec` a new argument carrying
that value; anything else is deleted. The last statement of a function is kept.
"""
...
Cut array arguments to smaller extents, dimensions sharing an extent together first.
"""
...
Shrink integer literals above one towards one and drop chunks of elements from array and
tuple literals and of arguments from calls, in every definition.
"""
...
# Where a definition can be cut, each with the path of child indices leading there.
function sites(e, path = Int[], out = Pair{Vector{Int}, Any}[])
    if e isa Integer && !(e isa Bool) && e > 1
        push!(out, path => e)
    elseif e isa Expr && !(e.head in (:quote, :macrocall))
...
The program with each of runtime activity, strong zero and the inline ABI switched on, for
the repro's header.
"""
...
"""
    ingredients(p, o, target, known)

What the program does without each of its statements and active arguments, one line each.
"""
...
# Apply CLEANUPS then PASSES, each to a fixpoint, until nothing changes.
...
What proposal `q` changes in `p`, with the call it is made under; a rejected proposal with
the same key is not tried again.
"""
