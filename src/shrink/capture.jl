# Catching the failing call: `autodiff` calls `describe_call` while `CAPTURING[]` is set, and
# the script's top-level expressions run one at a time until one throws.
...
"""
    describe(mode, f, ret, args, m, dir)

Return `autodiff(mode, f, ret, args...)` as a [`Call`](@ref) in terms of the script module
`m`, arguments named after the globals of `m` bound to them (else `a1`, `a2`, ...) and their
values written by [`value`](@ref). Return a string saying why when the call cannot be
written as source.
"""
...
# Module `pm` as an expression in `m`: its name, a path from a module `m` sees, or a loaded package by id.
...
"""
    run_script(file; dir, pending = "")

Run the script's top-level expressions one at a time with capture on, up to the first that
throws; return its index, the last call described in it and the exception, or `nothing`.
With `pending`, each description is written there before its call runs.
"""
...
"""
    capture_failure(file, o)

Return the script's first failure as `(index, call, class)`, or `nothing`; a call the child
died in is read back from its pending file.
"""
