...
function value(x, m::Module)
    x isa Number && return Value(x)
    x isa AbstractArray{<:Number} && return Value(x isa Array ? copy(x) : collect(x))
    text = sprint(show, x; context = :module => m)
    parsed = Meta.parse(text; raise = false)
    return Meta.isexpr(parsed, (:error, :incomplete)) ? nothing : Value(nothing, text)
end
...
function add_toplevel!(out, e, file, strip)
    ...
    if Meta.isexpr(e, :toplevel)                                # `a = 1; b = 2` on one line
...
function name_the_function(p::Program)
    ...
    if src.head == :function && is_signature(src.args[1])
        ...
        def = normalize(Expr(:function, Expr(:call, name, ps...), src.args[2]))
    end
