# The program being reduced, its parsing from a script and its printing as source.

"""
    Value(data, text)

A captured value: `text` is source that rebuilds it and `data` is the number or `Array`
behind it, `nothing` for anything else.
"""
struct Value
...
Return `x` as a [`Value`](@ref) in module `m`: a number or array as a literal, a global of
`m` or a function of a loaded package by name, anything else serialised to `file`; `nothing`
...
"""
    Call(mode, f, ret, args, checked = false)

The captured `autodiff(mode, f, ret, args...)`; `f` is a name or the source of an anonymous
function. A `checked` call is made through [`check`](@ref).
"""
struct Call
    mode::Mode
    f::Union{Symbol, Expr}
...
assignments keyed by the statement `Expr` itself, which passes keep when rebuilding bodies.
"""
struct Program
    defs::Vector{Any}
    call::Call
...
The top-level expressions of a script with literal `include`s inlined, `using A, B` split
and docstrings dropped: as parsed, or without line numbers and with every function in
`function f(x) ... end` form. Both hold the same expressions in the same order.
"""
...
function add_toplevel!(out, e, file, strip)
    ...
    if Meta.isexpr(e, (:toplevel, :block))                      # `a = 1; b = 2` on one line
...
# `Base.remove_linenums!` that also drops the line a macro call carries.
...
# Make an anonymous or local function captured as source a top-level definition, named `entry` if it had no name.
...
# A lambda or a short single-expression function back on one line.
function compact(e)
    e isa Expr || return e
...
Return the definitions and argument values as one string and the `autodiff` line as another.
"""
