# A wrong derivative as a failure.

# Thrown by `check` when the derivative called `where` disagrees with finite differences.
...
Call `autodiff(mode, f, ret, args...)` and compare its derivative with central finite
differences of `f`: in forward mode the tangent of the result and of every `Duplicated`
argument, in reverse mode the gradient along one direction. Throw `WrongDerivative` when they
disagree, else return what `autodiff` returned. Only real floats and arrays of them are
compared.
"""
