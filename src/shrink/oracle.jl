# Running candidates and classifying how they end.

"""
    Class(kind, key, detail)

The identity of a failure. `kind` is the exception type (or `:pass`, `:setup`, `:crash`,
`:timeout`), `key` locates it and `detail` describes it; two classes are equal when `kind`
and `key` agree.

Enzyme's exceptions are keyed by the method they blame, other exceptions by the first line of
their message with numbers normalised, a wrong derivative by which derivative disagreed, and a
crash by its assertion or internal error message, if any.
"""
struct Class
    kind::Symbol
    key::String
...
normalize_message(s) = replace(s, r"Main\.\w+\." => "", r"%\d+" => "%N", r"0x[0-9a-fA-F]+" => "0xX", r"\d+" => "N")
...
function classify(@nospecialize(err::Exception))
    ...
    lines = filter(l -> !isempty(l) && l != "Enzyme execution failed.", strip.(split(msg, '\n')))
    line = String(chopprefix(get(lines, 1, ""), "$kind: "))
    err isa WrongDerivative && return Class(kind, err.where, line)
    err isa Union{Compiler.EnzymeError, Compiler.CustomRuleError} || return Class(kind, normalize_message(line), line)
    mi = hasfield(typeof(err), :mi) ? err.mi : nothing
    if mi isa Core.MethodInstance && mi.def isa Method
        method = string(mi.def.name)
        startswith(method, "#") && (method = normalize_message(method))       # only compiler-generated names carry counters
        return Class(kind, "in " * method, "in $method: $line")
    end
    return Class(kind, err isa Compiler.EnzymeInternalError ? normalize_message(line) : "", line)
end

function crash_class(log::AbstractString)
    lines = split(log, '\n')
    i = findlast(l -> occursin(r"Assertion|LLVM ERROR|EnzymeInternalError", l), lines)
    key = i === nothing ? "" : normalize_message(lines[i])
...
"""
    evaluate(base, setup, call, primal, expected)

Return the class of `call` after `setup` in a fresh module, or `:setup` when `setup` fails or
`primal` neither passes nor ends in `expected`.
"""
...
"""
    Oracle(dir; isolate = false, timeout = 600, workers = 1)

Run candidate programs and remember their classes. Every candidate is written to `dir` as
`<n>.jl` with its output in `<n>.log`. With `isolate`, candidates run in `workers` child
processes kept warm between them and replaced when one dies, and a candidate is killed after
`timeout` seconds.
"""
mutable struct Oracle
    ...
    const isolate::Bool
    timeout::Float64
    primal::Union{Nothing, Class}            # how the original's primal ends; a candidate's may end the same way or pass
    const seen::Dict{Tuple{String, String}, Tuple{String, Class}}
    count::Int
    const slots::Vector{Union{Nothing, Child}}   # one child per worker
    spare::Union{Nothing, Child}
    children::Int
    primal_time::Float64                     # the longest a job took to get through its setup and primal
end
Oracle(dir; isolate = false, timeout = 600.0, workers = 1) =
    Oracle(dir, isolate || workers > 1, timeout, nothing, Dict{Tuple{String, String}, Tuple{String, Class}}(), 0, fill(nothing, workers), nothing, 0, 0.0)
...
function Base.close(o::Oracle)
    for c in [o.slots; o.spare]
        c === nothing || close(c.proc)
    end
    fill!(o.slots, nothing)
    o.spare = nothing
    return
end

"""
    run_job(o, slot, base, job, args...)

Run `job` in the child of `slot`, passing it through `base.in` and reading the result from
`base.out`. A child that dies is replaced and the result is a `:crash` classified from its
log; one still running after its budget is killed and the result is a `:timeout`.
"""
function run_job(o::Oracle, slot::Int, base::String, job::Symbol, args...)
    serialize(base * ".in", (job, args))
    child = o.slots[slot]
    fresh = child === nothing || !process_running(child.proc)
    if fresh
        died = child !== nothing
        spare, o.spare = o.spare, nothing
        child = o.slots[slot] = @something(spare, spawn_child(o))
        o.spare = died ? spawn_child(o) : nothing
        readline(child.proc)                                         # the child says when Enzyme is loaded
    end
    println(child.proc, base)
    flush(child.proc)
    out, start = base * ".out", time()
    budget = job == :evaluate && !fresh ? min(o.timeout, max(10.0, 3o.primal_time)) : o.timeout   # a fresh child compiles first
    log = base * ".log"
...
# Whether the process writing `log` has reported a fatal signal; it may hang in its crash handler.
...
# Run `job` (`:evaluate`, `:record_run` or `:capture_here`) with `args`, here or in a child.
perform(o::Oracle, base::String, job::Symbol, args...; slot::Int = 1) = o.isolate ? run_job(o, slot, base, job, args...) : perform_here(base, job, args)
...
# The child's loop: for each base path on stdin, run the job in `base.in` and write the result to `base.out`.
function serve()
    println("ready")
    flush(stdout)
...
# Run `p` as `dir/<id>.jl` and return `(id, class)`; a program whose source was seen before is answered from memory.
function (o::Oracle)(p::Program, slot::Int = 1)
    key = source(p)
    ...
    return o.seen[key] = (id, perform(o, base, :evaluate, setup, call, primal, o.primal; slot))
end

# Classify several programs, as many at a time as there are workers, each in its own child.
function (o::Oracle)(ps::Vector{Program})
    out = Vector{Tuple{String, Class}}(undef, length(ps))
    for window in Iterators.partition(eachindex(ps), length(o.slots))
        @sync for (slot, i) in enumerate(window)
            @async out[i] = o(ps[i], slot)
        end
    end
    return out
end
...
# Record the first value seen for `key`, copying arrays so later mutation does not reach them; return `x`.
...
function record_run(base::String, setup::String, primal::String)
    ...
    for (key, x) in RECORDS
        v = Base.invokelatest(value, x, m, joinpath(dirname(base), "values", "$key.jls"))
...
Run the primal once with every assignment instrumented; return `p` with the recorded values
attached and the class of that run.
"""
