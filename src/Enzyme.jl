...
"""
    shrink(file; isolate = false, timeout = nothing) -> path or nothing

Reduce the first failing `autodiff` call in the script `file` to a minimal reproducer, written
to `shrink_<time>/repro.jl` next to the script; `nothing` if no call failed.  The directory
...
`<n>.log`, and each accepted step under `checkpoints/`.
...
by a literal path) is reduced; a closure over locals is reported as not failing in isolation.
...
after `timeout` seconds, by default five times the original's own run and at least 60.
"""
function shrink end
